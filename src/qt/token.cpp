...
bool ToHash160(const std::string& strWormAddress, std::string& strHash160)
{   
    CTxDestination wormAddress = DecodeDestination(strWormAddress);
    if(IsValidDestination(wormAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&wormAddress);
...
bool ToWormAddress(const std::string& strHash160, std::string& strWormAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strWormAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToWormAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToWormAddress(tokenEvent.receiver, tokenEvent.receiver);
