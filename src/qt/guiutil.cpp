...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    // and this is the only place, where this address is supplied.
     widget->setPlaceholderText(QObject::tr("Enter a Worm address (e.g. %1)").arg("LhfHsuK9Ekjj2dYoEy4wvEiQHvRtzZQnYa"));
#endif
...
{
    // return if URI is not valid or is no WORM: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::WORM, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::WORM, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open worm.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WORM.lnk";
}
...
{
    return GetAutostartDir() / "worm.desktop";
}
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the worm app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add worm app to startup item list
