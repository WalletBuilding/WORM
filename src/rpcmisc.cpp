...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total worm balance of the wallet\n"
            "  \"darksend_balance\": xxxxxx, (numeric) the anonymized worm balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in worm/kB\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in worm/kB\n"
...
UniValue getstateinfo(const UniValue& params, bool fHelp)
{
    ...
    WormDGP wormDGP(globalState.get());

    UniValue obj(UniValue::VOBJ);
    obj.push_back(Pair("maxblocksize", (int)wormDGP.getBlockSize(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", (int)wormDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("mingasprice", (int)wormDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"wormaddress\"\n"
            "\nReturn information about the given worm address.\n"
            "\nArguments:\n"
            "1. \"wormaddress\"     (string, required) The worm address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"wormaddress\",     (string) The worm address validated\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are worm addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) worm address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"wormaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"wormaddress\"  (string, required) The worm address to use for the signature.\n"
...
UniValue getaddressbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "}\n"
            "\nResult: {\n"
            "  \"balance\",  (number) The current balance (in WORM satoshis)\n"
