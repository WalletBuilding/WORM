...
#include "wormupdater/wormupdater_p.h"

std::atomic<bool> running(false);
std::atomic<bool> wasCanceled(false);
QPointer<QWidget> win;
QtWormUpdater::UpdateController::DisplayLevel gDisplayLevel;
QtWormUpdater::ProgressDialog *gUpdatesProgress;
...
WormUpdater *UpdateController::updater() const
{
	...
	if(!d->mainUpdater->checkForUpdates()) {
		...
		if(gDisplayLevel >= ExtendedInfoLevel) {
			...
			if(gDisplayLevel >= ProgressLevel) {
				...
				d->checkUpdatesProgress->open(d->mainUpdater, &QtWormUpdater::WormUpdater::abortUpdateCheck);
			}
		}
		return true;
	}
...
void UpdateController::checkUpdatesDone(bool hasUpdates, bool hasError)
{
	...
	if(wasCanceled) {
		...
		if(hasUpdates) {
			...
			if(hasError) {
				qCWarning(logWormUpdater) << "maintenancetool process finished with exit code"
...
{
	return QIcon::fromTheme(QStringLiteral("system-software-update"), QIcon(QStringLiteral(":/res/icons/wormupdate.ico")));
}
...
UpdateControllerPrivate::UpdateControllerPrivate(UpdateController *q_ptr, const QString &version, QWidget *window) :
	q(q_ptr),
	window(window),
	mainUpdater(version.isEmpty() ? new WormUpdater(q_ptr) : new WormUpdater(version, q_ptr)),
	runAdmin(true),
	adminUserEdit(true),
	...
	scheduler(new SimpleScheduler(q_ptr))
{
	...
	QObject::connect(mainUpdater, &WormUpdater::checkUpdatesDone,
...
{
	if(running)
		qCWarning(logWormUpdater) << "UpdaterController destroyed while still running! This can crash your application!";
