...
bool WormDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule WormDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t WormDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t WormDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t WormDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t WormDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool WormDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void WormDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void WormDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void WormDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void WormDGP::createParamsInstance(){
...
dev::Address WormDGP::getAddressForBlock(unsigned int blockHeight){
...
void WormDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void WormDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void WormDGP::parseStorageOneUint64(uint64_t& value){
...
void WormDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule WormDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
