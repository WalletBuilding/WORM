...
WormState::WormState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = WormState::openDB(_path + "/wormDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

WormState::WormState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute WormState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, WormTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createWormAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            worm::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> WormState::vins() const // temp
{
...
void WormState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* WormState::vin(dev::Address const& _a) const
{
    return const_cast<WormState*>(this)->vin(_a);
}

Vin* WormState::vin(dev::Address const& _addr)
{
...
// void WormState::commit(CommitBehaviour _commitBehaviour)
// {
...
void WormState::kill(dev::Address _addr)
{
...
void WormState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
const dev::Address WormState::createWormAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void WormState::deleteAccounts(std::set<dev::Address>& addrs){
...
void WormState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void WormState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
